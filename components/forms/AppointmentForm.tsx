...
import { Appointment } from "@/store/patients";
...
export const AppointmentForm = ({
  ...
  const createAppointment = usePatientData((state) => state.createAppointment);

  const AppointmentFormValidation = getAppointmentSchema(type);
  const { appointments, patients } = usePatientData();
    ...
    try {
      if (type === "create" && patientId) {
        ...
        if (newAppointmentId) {
          form.reset();
          router.push(
            `/patients/${userId}/new-appointment/success?appointmentId=${newAppointmentId}`
          );
        }
      } else {
