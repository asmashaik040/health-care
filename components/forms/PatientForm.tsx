...
export const PatientForm = () => {
  ...
  const onSubmit = (values: z.infer<typeof UserFormValidation>) => {
    ...
    try {
      const user = {
        $id: "CP_" + values.name.replaceAll(" ", "_"),
        name: values.name,
        email: values.email,
        phone: values.phone,
      };
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="flex-1 space-y-6">
        <section className="mb-12 space-y-4">
          <h1 className="header">Need an appointment?</h1>
          <p className="text-dark-700">Follow simple steps & Get started with appointments.</p>
        </section>
